...
class RandomGenerator:
  ...
  def random_normal(self, shape, mean=0., stddev=1., dtype=None):
    ...
    return tf.random.normal(
        shape=shape, mean=mean, stddev=stddev, dtype=dtype, seed=self._seed)
  ...
  def random_uniform(self, shape, minval=0., maxval=None, dtype=None):
    ...
    return tf.random.uniform(
        shape=shape, minval=minval, maxval=maxval, dtype=dtype, seed=self._seed)
  ...
  def dropout(self, inputs, rate, noise_shape=None):
    self._maybe_init()
    if self._generator or self._seed is not None:
